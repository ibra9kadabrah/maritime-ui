...
// Helper Schemas (Copied from noonFormTypes.ts for consistency)
...
// Main Zod Schema for Arrival Report
export const arrivalReportSchema = z.object({
  ...
  fwdDraft: requiredPositiveNumber.max(30, { message: 'Draft must be between 0 and 30' }),
  aftDraft: requiredPositiveNumber.max(30, { message: 'Draft must be between 0 and 30' }),

  // Navigation - EOSP
  eospDate: requiredString,
  eospTime: requiredString.regex(VALIDATION_PATTERNS.TIME_HHMM, { message: "Time must be HH:MM" }),
  eospLatitude: requiredNumber.min(0).max(90, { message: 'Latitude must be 0-90' }),
  eospLatitudeDir: z.enum(['N', 'S']),
  eospLongitude: requiredNumber.min(0).max(180, { message: 'Longitude must be 0-180' }),
  eospLongitudeDir: z.enum(['E', 'W']),
  eospCourse: requiredInteger.min(0).max(359, { message: 'Course must be 0-359' }), // Added EOSP Course

  // Navigation - ETB (Added)
  etbDate: requiredString, // Assuming YYYY-MM-DD
  etbTime: requiredString.regex(VALIDATION_PATTERNS.TIME_HHMM, { message: "Time must be HH:MM" }),

  // Navigation - Harbour Steaming
  harbourDistance: requiredPositiveNumber,
  harbourTime: requiredString.regex(VALIDATION_PATTERNS.TIME_HHMM, { message: "Time must be HH:MM" }),
  ...
  // harbourLSIFO: requiredPositiveNumber,
  // harbourLSMGO: requiredPositiveNumber,
  ...
  // --- Engine/Machinery Section --- (Using same structure as others)
  // Main Engine Parameters
  engineLoadFOPressure: requiredPositiveNumber,
  engineLoadLubOilPressure: requiredPositiveNumber,
  engineLoadFWInletTemp: requiredNumber,
  engineLoadLOInletTemp: requiredNumber,
  engineLoadScavAirTemp: requiredNumber,
  engineLoadTCRPM1: requiredInteger.positive({ message: "TC RPM #1 must be positive" }),
  engineLoadTCRPM2: optionalInteger,
...
export type ProcessedArrivalFormData = z.infer<typeof arrivalReportSchema>;

// --- Remove Old Interface, Defaults, and Validation Rules ---
// export interface ArrivalReportFormData { ... } // Removed
// export const defaultArrivalValues: ArrivalReportFormData = { ... }; // Removed (moved to constants file)
// export interface ValidationRule { ... }; // Removed
// export type ValidationRules<T> = { ... }; // Removed
// export const arrivalValidationRules: ValidationRules<ArrivalReportFormData> = { ... }; // Removed
